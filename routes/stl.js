...
function query_dbs(res, fullName) {
	...
	query = "select P.Name, SP.STL from SeasonPerformance SP natural join Player P natural join PlaysFor natural join Season S where S.Start_Year = " + fullName + " order by SP.STL desc limit 10";
...
function output_persons(res,fullName,results) {
	res.render('stl.jade',
		   { title: "Person with STL " + fullName,
		     results: results }
	  );
}
