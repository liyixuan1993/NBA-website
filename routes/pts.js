...
function query_dbs(res, fullName) {
	...
	query = "select P.Name, SP.PTS from SeasonPerformance SP natural join Player P natural join PlaysFor natural join Season S where S.Start_Year = " + fullName + " order by SP.PTS desc limit 10";
...
function output_persons(res,fullName,results) {
	res.render('pts.jade',
		   { title: "Person with PTS " + fullName,
		     results: results }
	  );
}
