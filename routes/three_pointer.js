...
function query_dbs(res, fullName) {
	...
	query = "select P.Name, SP.Three_PM from SeasonPerformance SP natural join Player P natural join PlaysFor natural join Season S where S.Start_Year = " + fullName + " order by SP.Three_PM desc limit 10";
...
function output_persons(res,fullName,results) {
	res.render('three_pointer.jade',
		   { title: "Top 10 three_pointer player " + fullName,
		     results: results }
	  );
}
